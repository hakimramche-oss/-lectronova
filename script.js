...
function initGame() {
    ...
    els.pauseBtn.classList.remove('hidden'); // Show pause button
    els.pauseBtn.onclick = togglePause; // Ensure event is bound
...
function stopGame() {
    ...
    els.pauseBtn.classList.add('hidden'); // Hide pause button in menu
...
function resumeGame() {
    ...
    if (!audioState.musicMuted) els.bgMusic.play().catch(e => { });
}

function togglePause() {
    // Only allow pause if playing (level view is visible)
    if (els.levelView.classList.contains('hidden')) {
        return;
    }
...
document.addEventListener('click', () => {
    ...
    if (els.bgMusic.paused && !audioState.musicMuted) {
        startMusic();
    }
}, { once: true });
...
if (els.pauseBtn) {
    ...
    // Add touch listener for better responsiveness on touch screens
    els.pauseBtn.addEventListener('touchstart', (e) => {
        e.preventDefault();
        togglePause();
    }, { passive: false });
}
els.resumeBtn.onclick = resumeGame;
els.leaveBtn.onclick = () => {
    resumeGame(); // Just to clean up overlays
    stopGame();
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape') {
        if (currentState.isPaused) {
            resumeGame();
        } else if (!els.levelView.classList.contains('hidden')) {
            togglePause();
        }
    }
});
